...
int readLineError() {
    ...
    if (sensor2) result -= 2;  // 오른쪽 바깥
    if (sensor3) result -= 1;  // 오른쪽 중간
    if (sensor4) result += 1;  // 왼쪽 중간
    if (sensor5) result += 2;  // 왼쪽 바깥

    return result;  // 0이면 정중앙, 음수면 오른쪽 치우침, 양수면 왼쪽 치우침
}
...
int Search_Straight(int sp) {
    ...
    while(1) {
       ...
       /* else{
        ...
        int error = readLineError();  // 위 함수 사용
        int adjust = error * 50;     // 비례 게인값 (튜닝 가능)
